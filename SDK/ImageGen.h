...
extern "C" { /* using a C++ compiler */
  ...
  IMAGE_GEN_API void Generate_FresnelLens(unsigned char* Array, unsigned char* WFC, int width, int height, int depth, int centerX, int centerY, int radius, float Power, int cylindrical,
  ...
  IMAGE_GEN_API int Initialize_RegionalLUT(int width, int height, int depth, int NumBoards);

  IMAGE_GEN_API int Load_RegionalLUT(const char* const RegionalLUTPath, float* Max, float* Min, int board);

  IMAGE_GEN_API int Apply_RegionalLUT(unsigned char *Array, int board);
  ...
  IMAGE_GEN_API int GetBESTAxialPSF(float* axialAmplitude, float* Intensity, float Period, float OuterDiameter, float InnerDiameter);
